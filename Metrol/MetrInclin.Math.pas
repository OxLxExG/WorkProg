...
{  TInclinMetr3x4 = record
  ...
  public
   type
    ...
    class procedure SetupRoll(FirstStep: integer; Zen, Azim: Double; trr: variant); static;
    class procedure SolvRoll(const Y: TRollData; out rep: TResultSolvRoll); static;
    class function AddStep(Step: integer; const Info: string; trr: variant): Variant; static;
...
begin
  if Length(FData) = 0 then raise ESumDataException.Create('Нет данных для расчета медианы');
...
begin
  if FCount = 0 then raise ESumDataException.Create('Нет данных для расчета результата осреднения');
...
procedure TAngleMedianSum.Add(Data: Double);
 var
  d: Double;
begin
  if Length(FData) = 0 then FOld := Data;
  d := FOld - Data;
  if abs(d) < 180 then inherited Add(Data)
  else if d > 0 then inherited Add(Data + 360)
       else inherited Add(360 - Data)
end;
...
class function TMetrInclinMath.AddStep(Step: integer; const Info: string; trr: variant): Variant;
  function AddHG(stp: Variant; const Root: string):Variant;
  begin
    Result := TXMLScriptMath.AddXmlPath(stp, Root);
...
begin
  ...
  Result.STEP := Step;
  Result.INFO := Format('%d) %s',[step, Info]);
  AddHG(Result, 'accel');
  AddHG(Result, 'magnit');
  TXMLScriptMath.AddXmlPath(Result, 'TASK');
end;

class procedure TMetrInclinMath.SetupRoll(FirstStep: integer; Zen, Azim: Double; trr: variant);
 var
  i: Integer;
  s: Variant;
begin
  s := AddStep(FirstStep, Format('Установить Зенит %1.2f, Азимут %1.1f, визирный угол %d градусов.',[Zen, Azim, 0]), trr);
  s.TASK.Vizir_Stol := 0;
  s.TASK.Dalay_Kadr := 5;
  ...
  for I := 1 to 35 do
   begin
    s := AddStep(FirstStep+i, Format('стол: Установить визирный угол %d градусов.',[i*10]), trr);
    s.TASK.Vizir_Stol := i*10;
    s.TASK.Dalay_Kadr := 5;
   end;
end;

class procedure TMetrInclinMath.ExecStepIncl_OLD(stp: Integer; alg, trr: Variant);
...
class procedure TMetrInclinMath.ExportP3ToCalc(const TrrFile: string; NewTrr: Variant);
  const
   TBL_ZU_AZ: array[0..5] of Integer = (5,10,30,60,90,120);
   TBL_ZU_ALL: array[0..6] of Integer = (0,5,10,30,60,90,120);
begin
  if not TVxmlData(NewTrr.P_3).Node.HasAttribute('DevName') or
...
class procedure TMetrInclinMath.ExportP2ToCalc(const TrrFile: string; NewTrr: Variant);
  const
   TBL_ZU_AZ: array[0..1] of Integer = (45,90);
   TBL_ZU_ALL: array[0..2] of Integer = (0,45,90);
begin
  if not TVxmlData(NewTrr.P_2).Node.HasAttribute('DevName') or
     not TVxmlData(NewTrr).Node.ParentNode.ParentNode.HasAttribute(AT_SERIAL) then
     raise EExportReportException.Create('Параметры поверки не установлены');
  TThread.CreateAnonymousThread(procedure
   var
...
class procedure TMetrInclinMath.ExportP1ToCalc(const TrrFile: string; NewTrr: Variant);
  const TBL_Z: array[0..3,0..7]of Integer = ((1, 2,3,4,5,6,7,8),
       ...
       TBL_A2: array[0..11,0..1]of Integer = ((104,	81),
                                              ...
                                              (93,  92));
begin
  if not TVxmlData(NewTrr.P_1).Node.HasAttribute('DevName') or
     not TVxmlData(NewTrr).Node.ParentNode.ParentNode.HasAttribute(AT_SERIAL) then
     raise EExportReportException.Create('Параметры поверки не установлены');
  TThread.CreateAnonymousThread(procedure
   var
    ...
    try
     ...
     try
      ...
      for i := 0 to 3 do for j := 0 to 7 do
       begin
        st := XToVar(GetXNode(TVxmlData(NewTrr).Node, 'P_1.STEP'+TBL_Z[i,j].ToString));
      ...
      for i := 0 to 11 do for j := 0 to 3 do
       begin
        st := XToVar(GetXNode(TVxmlData(NewTrr).Node, 'P_1.STEP'+TBL_A[i,j].ToString));
      ...
      for i := 0 to 11 do for j := 0 to 1 do
       begin
        st := XToVar(GetXNode(TVxmlData(NewTrr).Node, 'P_1.STEP'+TBL_A2[i,j].ToString));
        v[j, i] := Double(st.азимут.CLC.VALUE);
        e[j, i] := Double(st.СТОЛ.err_азимут);
       end;
...
class procedure TMetrInclinMath.ImportIncFile(const TrrFile: string; NewTrr: Variant);
 var
  Convert: TConvert;
  ss: TstringList;
  v: Variant;
...
begin
  v := XtoVar(TVxmlData(NewTrr).Node.ParentNode);
...
class procedure TMetrInclinMath.ExportToInc(const TrrFile: string; NewTrr: Variant);
 var
  Convert: TConvert;
  sernom: Integer;
begin
...
begin
  ...
  TXMLScriptMath.TrrVect3D(trr.magnit.m3x4, incl.magnit);//, 1000);
...
class procedure TMetrInclinMath.FindZenViz(incl, trr: Variant);
 var
  o, zu, x,y,z: Double;
begin
  TXMLScriptMath.TrrVect3D(trr.accel.m3x4, incl.accel);
