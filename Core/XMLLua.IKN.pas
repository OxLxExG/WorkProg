...
uses  tools, debug_except, MathIntf, System.UITypes, XMLLua, XMLLua.math, RootImpl, VerySimple.Lua.Lib,
...
//    class function CallMeth(Instance: TObject; ClassType: TClass; const MethodName: String; var Params: Variant): Variant;
    class procedure Exec_IKN_A2(v, t: IXMLNode); overload; static;
    class procedure Setup_IKN_A2(v: IXMLNode); overload; static;
    class procedure Setup_IKN(t: IXMLNode); overload; static;
    class procedure Import_IKN_A2(const TrrFile: string; NewTrr: IXMLNode); overload; static;
  published
    class function Exec_IKN_A2(L: lua_State): Integer; overload; cdecl; static;
    class function Setup_IKN_A2(L: lua_State): Integer; overload; cdecl; static;
    class function Setup_IKN(L: lua_State): Integer; overload; cdecl; static;
    class function Import_IKN_A2(L: lua_State): Integer; overload; cdecl; static;
  end;
...
begin
  TXMLLua.RegisterLuaMethods(TXMLScriptIKN);
...
class procedure TXMLScriptIKN.Setup_IKN(t: IXMLNode);
 var
  i,f, n: Integer;
  root: IXMLNode;
begin
...
class procedure TXMLScriptIKN.Setup_IKN_A2(v: IXMLNode);
 var
  l, p, f, m: IXMLNode;
  i, j: Integer;
 const
...
begin
  p := v.AddChild(T_PRV, 1);// TXMLScriptMath.AddXmlPath(v, 'ѕроводимось');
  p.Attributes['SIZE'] := 0;
  for j := 1 to 4 do
   begin
...
class procedure TXMLScriptIKN.Exec_IKN_A2(v, t: IXMLNode);
 var
  root: IXMLNode;
  const
   PFT = '‘аза.‘%d.%s%d.T.DEV';
...
class procedure TXMLScriptIKN.Import_IKN_A2(const TrrFile: string; NewTrr: IXMLNode);
 var
  ss: TStrings;
  root: IXMLNode;
  i,f: Integer;
...
class function TXMLScriptIKN.Exec_IKN_A2(L: lua_State): Integer;
begin
  Exec_IKN_A2(TXMLLua.XNode(L,1), TXMLLua.XNode(L,2));
  Result := 0;
end;

class function TXMLScriptIKN.Import_IKN_A2(L: lua_State): Integer;
begin
  Import_IKN_A2(string(lua_tostring(L,1)),  TXMLLua.XNode(L,2));
  Result := 0;
end;

class function TXMLScriptIKN.Setup_IKN(L: lua_State): Integer;
begin
  Setup_IKN(TXMLLua.XNode(L,1));
  Result := 0;
end;

class function TXMLScriptIKN.Setup_IKN_A2(L: lua_State): Integer;
begin
  Setup_IKN_A2(TXMLLua.XNode(L,1));
  Result := 0;
end;
